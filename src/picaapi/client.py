...
class Client:
    ...
    async def request(self, method: str, dir: str, json = None):
        '''
        发送一般哔咔API请求。
        ...
        Raises:
            PicaAPIError: 请求失败
        '''
        response = await self.client.request(method=method, url=dir, json=json, headers=makeHeaders(dir, method, self.token, self.quality))
        json = response.json()
        if json['code'] != 200:
    ...
    async def comic(self, id: str) -> ComicDetailed:
        '''
        获取漫画详细信息。
        ...
        '''
        response = await self.request('GET', f'comics/{id}')
        return ComicDetailed(response['comic'])
    
    async def advancedSearch(self, keyword: str, categories: list[str] = [], sort: str = 'dd', page: int = 1) -> Page:
        '''
        哔咔高级搜索。
    ...
    async def childrens(self, comment_id: str, page: int = 1) -> Page:
        '''
        查看某评论的评论。
        ...
        Returns:
            Page: 返回结果页，数据类型为Comment
        '''
        response = await self.request('GET', f'comments/{comment_id}/childrens?page={page}')
    ...
    async def categoryComic(self, category: str, sort: str = 'dd', page: int = 1) -> Page:
        '''
        从分类获取漫画列表。
    ...
    async def likeComic(self, comic_id: str) -> None:
        '''
        点赞漫画。
    ...
    async def favouriteComic(self, comic_id: str) -> None:
        '''
        收藏漫画。
    ...
    async def likeComment(self, comment_id: str) -> None:
        '''
        点赞评论。
    ...
    async def commentComic(self, comic_id: str, content: str) -> None:
        '''
        评论漫画。
    ...
    async def commentComment(self, comment_id: str, content: str) -> None:
        '''
        评论评论。
